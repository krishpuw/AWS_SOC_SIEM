...
def calculate_enriched_severity(original_severity: str, vt: dict, abuse: dict) -> str:
    ...
    Rules:
      - AbuseIPDB confidence >= 80  → at least HIGH
      - VirusTotal malicious >= 5   → at least HIGH
      - AbuseIPDB confidence >= 50  → at least MEDIUM
      - Is Tor exit node            → at least MEDIUM
    """
...
def enrich_alert(alert: dict) -> dict:
    """
    Run VT + AbuseIPDB queries for the source IP in this alert.
    Returns enrichment payload to write back to DynamoDB.
    """
...
def lambda_handler(event, context):
    """
    Entry point. Can be triggered two ways:
      1. Scheduled (EventBridge rule) — enriches all unenriched alerts in DynamoDB
    ...
    # ── Mode 1: single alert passed directly ──────────────────────────────────
    ...
    # ── Mode 2: scheduled — process all unenriched alerts ─────────────────────
    else:
...
if __name__ == "__main__":
    ...
    os.environ["DYNAMODB_TABLE_NAME"] = "security-alerts"

    # Pass a real API key via env to test live queries, otherwise it gracefully errors
    print("=== Threat Intel Lambda — Local Test ===\n")

    # Test 1: known malicious IP (Shodan scanner)
    ...
    # Test 2: scheduled enrichment against local DynamoDB
    print("\n--- Running full lambda_handler (scheduled mode) ---")
